...
func NewSubFtCosi(n *onet.TreeNodeInstance, vf VerificationFn, suite cosi.Suite) (onet.ProtocolInstance, error) {
	...
	// TODO: Subcommitment channel not for BLS protocol.
	if n.IsRoot() {
...
func (p *SubFtCosi) Dispatch() error {
	...
	switch p.suite.(type) {
	case pairing.Suite:
		log.Lvl3("Dispatch BLS Cosi")
		return DispatchSubProtocolBLS(p, announcement)
	default:
		log.Lvl3("Dispatch default CoSi")
		return DispatchSubProtocolDefault(p, announcement)
	}
}

func DispatchSubProtocolBLS(p *SubFtCosi, announcement StructAnnouncement) error {
	...
	if p.IsRoot() {
		...
		pairing_suite, ok := p.suite.(pairing.Suite)
		signaturePoint, finalMask, err := generateSignature(pairing_suite, p.TreeNodeInstance, p.Publics, responses, p.Msg, ok)

		if err != nil {
			return err
		}

		tmp, err := PointToByteSlice(pairing_suite, signaturePoint)
...
func DispatchSubProtocolDefault(p *SubFtCosi, announcement StructAnnouncement) error {
...
loop:
	...
	if isInMask {
		...
		tmp, err := ScalarToByteSlice(p.suite, personalResponse)
		if err != nil {
			return err
		}
		responses = append(responses, StructResponse{p.TreeNode(), Response{CoSiReponse: tmp}})
	}
	...
	tmp, err := ScalarToByteSlice(p.suite, aggResponse)
	if err != nil {
		return err
	}

	// send to parents
	err = p.SendToParent(&Response{CoSiReponse: tmp})
	if err != nil {
		return err
	}
	return nil
